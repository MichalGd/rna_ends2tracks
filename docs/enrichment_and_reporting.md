...
This is a scope check, not a statement that APA-B is unfinished. The audited `biolserv` GRCm39 QuantSeq REV single-end deployment satisfies the gate when its accepted manifests are configured. GRCh38 and paired-end APA-B do not inherit that acceptance automatically.
