...
This creates a release-specific environment such as `/opt/conda_envs/rna_ends2tracks-apa-b-0.1.0a10.post2` without changing the stable main launcher. It is safe to install beside an active main-workflow run if disk and memory headroom are adequate. APA-B remains disabled until the synthetic pilot and a real QuantSeq REV canary for each intended assembly pass. Follow [the APA-B pilot contract](POLYASEQTRAP_ADAPTER_CONTRACT.md); do not set `APA_B_PILOT_ACCEPTED=true` merely because installation succeeded.
