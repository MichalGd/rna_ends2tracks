...
| `apa_b` | Independently clusters raw C0-equivalent endpoints with PolyAseqTrap, filters candidates with DeepIP, quantifies its own PAS catalog and tests usage with DRIMSeq/stageR. Receipt-validated C1+C1S may reconstruct the unchanged raw endpoint universe without reusing APA-A filtering. | `07_apa_b/` independent catalog/count matrix, DRIMSeq/stageR tables, NA/fit audits, gene summaries and provenance. | Enabled in the audited GRCm39 single-end new-project template. Disabled by `RUN_APA_B=false`; any enabled run requires installation and validation manifests covering its exact protocol/assembly scope. |
